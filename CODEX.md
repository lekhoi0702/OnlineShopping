...
## 0.7 Language And Encoding Rules

**Write language text directly and preserve readable encoding.**

Rules:

* Use real text for Vietnamese and Traditional Chinese, for example `Tiếng Việt` and `中文`.
* Do not write Unicode escape text like `Ti\u1ebfng Vi\u1ec7t` or `\u4e2d\u6587` unless the existing file format strictly requires it.
* Preserve the file's existing encoding when editing.
* If a file already has encoding issues, do not rewrite the whole file. Make the smallest safe change and report the encoding issue.
* Do not convert readable language text into escaped Unicode during edits.
